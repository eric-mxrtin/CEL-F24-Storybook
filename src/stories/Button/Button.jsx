...
export const Button = ({ label, type, color, disabled }) => {
  ...
  return (
    <button
      type="button"
      class={`py-3.5 px-4 inline-flex items-center gap-x-2 text-sm text-color-white font-semibold rounded-lg border border-transparent bg-${color}-500 text-white hover:bg-${color}-700 focus:outline-none focus:bg-${color}-700 disabled:opacity-50 disabled:pointer-events-none`}
      disabled={disabled}
    >
      {label}
    </button>
